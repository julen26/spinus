...
sp.DefaultShader = function (context, hasTexture) {
    ...
    shader.compile(
        ...
        +       '\n#endif\n'
        +       'mat4 transform = u_transforms[0];'
        +       'for (int i = 1; i < 4; i++) {'
        +           'transform = u_transforms[i] * transform;'
        +       '}'
        +       'gl_Position = vec4( (u_projection * u_view * transform * vec4(a_position, 0, 1)).xy, 0, 1);'
        +       'v_color = a_color;'
        +   '}'
        ,
